...
class(BOD$Time)
BOD$Time
#ggplot(BOD, aes(x=Time, y=demand)) + geom_bar(stat = "bin")
# Convert Time to a discrete (categorical) variable with factor() function.
ggplot(BOD, aes(x=factor(Time), y=demand)) + geom_bar(stat = "identity")
# WILL CAUSE no blank position for 6 on x axis
...
ggplot(diamonds[diamonds$carat<3,],aes(x=carat)) + geom_histogram(binwidth = .01)
...
ggplot(cabbage_exp,aes(x=interaction(Date,Cultivar),y=Weight))+
  ...
  geom_text(aes(y=Weight+0.1,label=Weight),colour='red')
...
uspopchange
help(rank)
uspopchange[order(-uspopchange$Change),]
help(order)
length(order(-uspopchange$Change))
help(geom_bin2d)
